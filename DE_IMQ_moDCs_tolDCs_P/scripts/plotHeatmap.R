# Plot Heatmap for moDCs and tolDCs with IMQ with GC and age annotations
...
plot_heatmap <- function(dds_object, DEresultsFile, metadata, figdir, plotName, columnContrast, clusterRows = F){
  ...
  ha_samples <- HeatmapAnnotation(Group = c(ordered_samples[,columnContrast]),
                                  col = list(Group = c('moDCIMQ' = '#40E0D0', 'tolDCIMQ' = '#a62d6b')))
  # logFC color
...
DEresultsFile = '/mnt/Citosina/amedina/lupus/RNA_lupus/DE_IMQ_moDC_tolDC_P/DE_files/DE_IMQ_moDC_tolDC_P.csv'
load('/mnt/Citosina/amedina/lupus/RNA_lupus/DE_IMQ_moDC_tolDC_P/dds_objects/dds_IMQ_moDC_tolDC_P.RData') # dds_mo
dds_sub
figdir <- '/mnt/Citosina/amedina/lupus/RNA_lupus/DE_IMQ_moDC_tolDC_P/figures/'
load(file = paste0(workdir, 'counts/txi.RData'))
plotName = 'IMQ_moDCs_tolDCs_P'
metadata <- metadata[(metadata$Group == 'SLE') & (metadata$Cell_type %in% c('moDCIMQ', 'tolDCIMQ')),]
...
plot_heatmap(dds_sub, DEresultsFile, metadata, figdir, plotName, columnContrast, clusterRows = F)
