# Plot Heatmap for Control samples by treatment
...
plot_heatmap <- function(dds_object, DEresultsFile, metadata, figdir, plotName, columnContrast, clusterRows = F){
  ...
  heatmap_figure <- Heatmap(ordered_mat, cluster_rows = clusterRows, cluster_columns = F, name = 'Z-score',
                            ...
                            top_annotation = c(ha_samples ,age_samples),
...
dir(paste0(workdir, 'DE_Type_Treatment_C/DE_files'))
...
figdir <- paste0(workdir, 'DE_Type_Treatment_C/figures/')

# ----monocyte vs moDC---

meta_sub <- control_df[control_df$Cell_type %in% c(comparisons[[1]]),]
meta_sub <- control_df[control_df$Cell_type %in% c(comparisons[[1]]),]
meta_sub$Group <- as.character(meta_sub$Group)
dds_sub <- dds_control[, meta_sub$sample_ID]
DEresultsFile = paste0(workdir, 'DE_Type_Treatment_C/DE_files/DE_control_',de_files[1],'.csv')
plotName = de_files[1]
...
meta_sub <- control_df[control_df$Cell_type %in% c(comparisons[[2]]),]
meta_sub <- control_df[control_df$Cell_type %in% c(comparisons[[2]]),]
meta_sub$Group <- as.character(meta_sub$Group)
dds_sub <- dds_control[, meta_sub$sample_ID]
DEresultsFile = paste0(workdir, 'DE_Type_Treatment_C/DE_files/DE_control_',de_files[2],'.csv')
plotName = de_files[2]
columnContrast = 'Cell_type'
...
meta_sub <- control_df[control_df$Cell_type %in% c(comparisons[[3]]),]
meta_sub <- control_df[control_df$Cell_type %in% c(comparisons[[3]]),]
meta_sub$Group <- as.character(meta_sub$Group)
dds_sub <- dds_control[, meta_sub$sample_ID]
DEresultsFile = paste0(workdir, 'DE_Type_Treatment_C/DE_files/DE_control_',de_files[3],'.csv')
plotName = de_files[3]
columnContrast = 'Cell_type'
...
meta_sub <- control_df[control_df$Cell_type %in% c(comparisons[[4]]),]
meta_sub <- control_df[control_df$Cell_type %in% c(comparisons[[4]]),]
meta_sub$Group <- as.character(meta_sub$Group)
dds_sub <- dds_control[, meta_sub$sample_ID]
DEresultsFile = paste0(workdir, 'DE_Type_Treatment_C/DE_files/DE_control_',de_files[4],'.csv')
plotName = de_files[4]
columnContrast = 'Cell_type'
